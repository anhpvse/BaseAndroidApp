...
class NonEmptySet<out T>
  ...
  private constructor(
    ...
    operator fun plus(l: NonEmptySet<@UnsafeVariance T>): NonEmptySet<T> = NonEmptySet(set + l.set)
    ...
    companion object {
       ...
       */
      @JvmStatic
      fun <T> Collection<T>.toNonEmptySetOrNull(): NonEmptySet<T>? = if (isEmpty()) null else NonEmptySet(toSet())
       ...
       */
      @JvmStatic
      fun <T> Set<T>.toNonEmptySetOrNull(): NonEmptySet<T>? =
        (this as? NonEmptySet<T>)
